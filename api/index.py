...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS customers (
                   ...
                   step6 TEXT DEFAULT 'Pending', step7 TEXT DEFAULT 'Pending',
                   step8 TEXT DEFAULT 'Pending', step9 TEXT DEFAULT 'Pending',
                   step10 TEXT DEFAULT 'Pending', step11 TEXT DEFAULT 'Pending',
                   step12 TEXT DEFAULT 'Pending', step13 TEXT DEFAULT 'Pending',
...
# Health check endpoint - always works even if DB is down
...
def health():
    ...
    try:
        conn = get_conn()
        conn.close()
        return jsonify({'status': 'ok', 'db': 'connected', 'url_prefix': masked})
    except Exception as e:
        return jsonify({'status': 'error', 'db': 'failed', 'error': str(e), 'url_prefix': masked}), 500
